...
example.
Tweaks Py3status inbuilt module by allowing different formats for volume
levels and a qualifier for max volume.
...
Configuration parameters:
    ...
    format: Format of the output.
        (default '♪: {percentage}%')
    format_good: Format of full volume output. Uses format if None.
        (default '♪: full')
    format_degraded: Format of degraded output.
        (default None)
...
class Py3status:
    ...
    button_mute = 0
    button_up = 0
    background = None       # specifically for i3-gaps background color
    use_color = True       # whether to use color outputs
    cache_timeout = 10
    channel = 'Master'
    device = 'default'
    format = u'♪: {percentage}%'
    ...
    # compares current volume to the thresholds, returns a color code and format
    def _perc_to_status(self, i3s_config, string):

        try:
            value = int(string)
        except ValueError:
            return i3s_config['color_bad'], self.format_muted

        if self.format_degraded is None:
            self.format_degraded = self.format
        if self.format_good is None:
            self.format_good = self.format

        if value <= 0:
            return i3s_config['color_bad'], self.format_muted
        elif value < self.threshold_bad:
            return i3s_config['color_bad'], self.format_degraded
        elif value < self.threshold_degraded:
            return i3s_config['color_degraded'], self.format_degraded
        elif value < 100:
            return None, self.format
        else:
            return i3s_config['color_good'], self.format_good
    ...
    def vol(self, i3s_output_list, i3s_config):
        ...
        # determine the color and format based on the current volume level
        color, format = self._perc_to_status(i3s_config, perc if not muted else '0')

        # format the output
        text = self._format_output(format, perc)
        # create response dict
        response = {
            'cached_until': time() + self.cache_timeout,
            'color': color if self.use_color else None,
