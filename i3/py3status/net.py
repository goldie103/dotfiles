...
Tweaked from py3status's wifi module to also show public ip and use i3-gaps
...
class Py3status:
    ...
    def get_wifi(self, i3s_output_list, i3s_config):
        ...
        if '{public_ip}' in self.format_up:
            try:
                public_ip = urlopen(self.url, timeout=self.timeout).read().decode('utf-8')
            except Exception:
                public_ip = None
        else:
            public_ip = ''
        ...
        response.update({self.indicator: color})
        return response
